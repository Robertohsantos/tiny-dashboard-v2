...
export function UserSessionsSettingsForm() {
  ...
  const handleEndSession = async (token: string) => {
    if (!token) {
      toast.error('Unable to end session: missing token.')
      return
    }

    const result = await tryCatch(
      api.session.revoke.mutate({
        body: {
          token,
        },
      }),
        ...
        <Annotated.Section>
          <div className="merge-form-section">
            ...
            {sessions.data?.map((session) => {
              ...
              const device = getDeviceInfoByUserAgent(userAgent)
              ...
              return (
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() => void handleEndSession(session.token)}
                  >
                    End session
                    <Trash2Icon />
                  </Button>
