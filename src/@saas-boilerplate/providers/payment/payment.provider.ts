...
{
  static instance: IPaymentProvider<never>
  ...
  async handle(request: Request): Promise<Response> {
    try {
      ...
      const { event: eventType, data } = event as {
        event: import('./providers/provider-adapter.interface').PaymentProviderAdapterEvent
        data: unknown
      }
      ...
      switch (eventType) {
        ...
        case 'customer.subscription.created': {
          ...
          const customer = await this.database.getCustomerById(d.customer)
          if (!customer) throw new Error('Customer not found')

          const price = await this.database.getPriceById(d.price)
          if (!price) throw new Error('Price not found')
          ...
          const subscription = await this.database.createSubscription({
            ...
            priceId: price.id,
            quantity: d.quantity,
            trialDays: d.trialDays ? 0 : undefined,
            metadata: d.metadata as Record<string, any> | undefined,
            billingCycleAnchor: d.billingCycleAnchor,
            prorationBehavior: d.prorationBehavior,
        ...
        case 'customer.subscription.updated': {
          ...
          if (!subscription) {
            console.error(
              `Subscription with providerId ${d.id} not found for update.`,
            )
            break
          }

          const price = await this.database.getPriceById(d.priceId)
          if (!price) throw new Error('Price not found')
        ...
        case 'customer.subscription.deleted': {
          ...
          if (!subscription) {
            console.error(
              `Subscription with providerId ${d.id} not found for deletion.`,
            )
            break
          }
        ...
        case 'customer.subscription.trial_will_end':
          if (this.events?.onSubscriptionTrialWillEnd && data && typeof data === 'object') {
            await this.events.onSubscriptionTrialWillEnd(data as Subscription)
          }
        ...
        case 'invoice.payment_succeeded':
          ...
          if (this.events?.onInvoicePaymentSucceeded && data && typeof data === 'object') {
            await this.events.onInvoicePaymentSucceeded(data as any)
          } else {
            if (data && typeof (data as { id?: string }).id === 'string') {
              console.log(`Pagamento da fatura ${(data as { id: string }).id} realizado com sucesso`)
            } else {
              console.log('Pagamento de fatura realizado com sucesso')
            }
          }
        ...
        case 'invoice.payment_failed':
          ...
          if (this.events?.onInvoicePaymentFailed && data && typeof data === 'object') {
            await this.events.onInvoicePaymentFailed(data as any)
          } else {
            if (data && typeof (data as { id?: string }).id === 'string') {
              console.log(`Falha no pagamento da fatura ${(data as { id: string }).id}`)
            } else {
        ...
        case 'customer.created':
          if (!data || typeof data !== 'object') break
          {
            const c = data as Partial<Customer> & { id: string }
            await this.database.updateCustomer(c.id, c as Partial<CustomerDTO>)
            if (this.events?.onCustomerCreated) await this.events.onCustomerCreated(c as Customer)
          }
        ...
        case 'customer.updated':
          if (!data || typeof data !== 'object') break
          {
            const c = data as Partial<Customer> & { id: string }
            await this.database.updateCustomer(c.id, c as Partial<CustomerDTO>)
            if (this.events?.onCustomerUpdated) await this.events.onCustomerUpdated(c as Customer)
          }
  ...
  private async syncPlanPrices(
    ...
    try {
      ...
      for (const priceData of pricesToSync) {
        ...
        const existingPrice = this.findPriceInPlan(plan, priceData)

        if (existingPrice) {
          // Update price in database
          await this.database.updatePrice(
            existingPrice.id,
            {
          ...
          if (this.events?.onPriceUpdated) {
            const refreshed = await this.database.getPriceById(existingPrice.id)
            if (refreshed) {
              await this.events.onPriceUpdated(refreshed)
            }
          }
        } else {
  ...
  static initialize<TPlans extends Omit<PlanDTO, 'providerId'>[]>(
    ...
    if (!PaymentProvider.instance) {
      PaymentProvider.instance = new PaymentProvider(params) as unknown as IPaymentProvider<TPlans>
    }

    return PaymentProvider.instance as IPaymentProvider<TPlans>
  }
}
