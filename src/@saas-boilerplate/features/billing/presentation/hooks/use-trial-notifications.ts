  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        const lastDismissedAtValue = parsed['lastDismissedAt']
        const isDismissedValue = parsed['isDismissed']

        const lastDismissedAt =
          typeof lastDismissedAtValue === 'string' ? lastDismissedAtValue : null
        const isDismissed =
          typeof isDismissedValue === 'boolean' ? isDismissedValue : false
