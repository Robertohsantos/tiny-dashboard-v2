...
type DemoDataContext = Pick<IgniterAppContext, 'services'>
...
export async function createDemoDataForOrganization(
  ...
  if (!database?.lead || !database?.submission) {
    throw new Error('Database service is unavailable for demo data creation')
  }
  const firstNames = [
    'Lucas',
    'Ana',
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let s = 0; s < nSubmissions; s++) {
      ...
      await database.submission.create({
        data: {
          organizationId,
          leadId: lead.id,
