...
const getContactEmail = (metadata: unknown): string => {
  if (!metadata || typeof metadata !== 'object' || metadata === null) {
    return ''
  }

  const metadataRecord = metadata as Record<string, unknown>
  const contact = metadataRecord['contact']
...
export function BillingChangeEmailSection() {
  const auth = useAuth()
  const defaultEmail = getContactEmail(auth.session.organization?.metadata)
