...
const hasScreenshots = (
  ...
  metadata: Integration['metadata'] & { screenshots: string[] }
} => Array.isArray(integration.metadata.screenshots)
...
export function IntegrationBanner({
  ...
  const featuredIntegrations = integrations
    ...
    .filter((integration) => featured.includes(integration.slug))
            ...
            <div className="[mask-image:linear-gradient(90deg,transparent,black_8%,black_92%,transparent)]">
              <CarouselContent>
                {featuredIntegrations.map((integration) => (
                  <CarouselItem key={integration.slug} className="basis-2/3">
                    <Link
