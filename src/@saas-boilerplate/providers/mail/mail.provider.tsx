...
> implements IMailProvider<TEmailTemplates>
{
  ...
  schedule<TSelectedTemplate extends keyof TEmailTemplates>(
    params: MailProviderSendParams<TEmailTemplates, TSelectedTemplate>,
    date: Date,
  ): Promise<void> {
    if (date.getTime() <= Date.now()) {
      return Promise.reject(new Error('Schedule date must be in the future'))
    }

    const timeout = date.getTime() - Date.now()
    setTimeout(() => {
      void this.send(params).catch((error) => {
