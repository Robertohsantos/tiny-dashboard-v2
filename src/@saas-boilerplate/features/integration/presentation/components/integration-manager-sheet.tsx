...
export function IntegrationManagerSheet({
  ...
  const integrationLogo =
    typeof integration.metadata.logo === 'string'
      ? integration.metadata.logo
      : null

  const metadataDefaults: Record<string, unknown> =
    integration.state?.metadata &&
    typeof integration.state.metadata === 'object'
      ? (integration.state.metadata as Record<string, unknown>)
      : {}
  ...
  const form = useFormWithZod({
    ...
    defaultValues: {
      ...
      ...metadataDefaults,
    } satisfies IntegrationFormValues,
    onSubmit: async (values) => {
      setIsInstalling(true)

      try {
        const { enabled, ...metadataValues } = values
